"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;

var _sLoading = _interopRequireDefault(require("./../s-loading.vue"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

// 暴露dialog函数  在main中use
var _default = {
  install: function install(Vue, options) {
    // Vue  接受参数vue实例对象
    // options  接受传入参数 配置项
    // 一 创建一个子类 参数是包含组件选项的对象
    var loading = Vue.extend(_sLoading["default"]); // 创建一个loading实例对象

    var profail = new loading({
      el: document.createElement('div')
    }); // 将对象的dom挂载到body上
    // 不能使用app
    // document.getElementById('app').appendChild(profail.$el)

    document.body.appendChild(profail.$el); // 插件接收值  options

    if (options) {
      // 在vue.use时判断参数 如果use时传入了参数 就jin行更改类型
      // 1.更改icon图标
      if (options.icon) {
        profail.icon = options.icon;
      } // 2.更改字体颜色


      if (options.color) {
        profail.color = options.color;
      } // 3.如需进行后续操作 可将其进行设置传入更多参数。。。。。。。。。。

    }

    var profailMethod = {
      /**
       * params： loading文本提示内容
       */
      open: function open(text) {
        if (text) {
          profail.text = text;
        }

        profail.show = true;
      },
      closed: function closed() {
        profail.show = false;
      }
    }; // 此处是给sloading添加open closed方法

    Vue.prototype.$sloading = profailMethod;
  }
};
exports["default"] = _default;